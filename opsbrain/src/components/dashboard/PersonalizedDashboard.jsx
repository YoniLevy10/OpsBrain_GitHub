...
export default function PersonalizedDashboard({ business, user }) {
  ...
  const loadUserConfig = async () => {
    try {
      const configs = await opsbrain.entities.DashboardConfig.filter({
  ...
  const saveUserConfig = async (newWidgets) => {
    try {
      ...
      if (configs.length > 0) {
        await opsbrain.entities.DashboardConfig.update(configs[0].id, configData);
      } else {
        await opsbrain.entities.DashboardConfig.create(configData);
      }
    } catch (error) {
