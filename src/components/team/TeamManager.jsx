...
export default function TeamManager() {
  ...
  const { data: members = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await base44.entities.WorkspaceMember.filter({ workspace_id: activeWorkspace.id });
    },
  ...
  const inviteMutation = useMutation({
    mutationFn: async (data) => {
      // שליחת מייל הזמנה דרך הפונקציה
      const emailResponse = await base44.functions.invoke('sendTeamInvitation', {
        email: data.email,
        role: data.role,
        workspaceName: activeWorkspace.name || activeWorkspace.business_name || 'מרחב העבודה שלך',
        inviterName: null, // הפונקציה תיקח את השם מהמשתמש המחובר
    ...
    onSuccess: () => {
      ...
      setInviteEmail('');
      toast.success(language === 'he' ? 'הזמנה נשלחה בהצלחה למייל' : 'Invitation email sent successfully');
    },
    onError: (error) => {
      console.error('Invitation error:', error);
      toast.error(language === 'he' ? 'שגיאה בשליחת הזמנה למייל' : 'Error sending invitation email');
    }
  });

  const removeMutation = useMutation({
    mutationFn: async (memberId) => {
      return base44.entities.WorkspaceMember.update(memberId, {
        status: 'removed',
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ memberId, role }) => {
      return base44.entities.WorkspaceMember.update(memberId, { role });
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['workspace-members']);
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader className="border-b bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setShowInviteDialog(true)} className="bg-blue-600 hover:bg-blue-700 min-w-[180px]">
