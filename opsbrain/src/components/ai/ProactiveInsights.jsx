...
export default function ProactiveInsights() {
  ...
  const { data: invoices = [] } = useQuery({
    queryKey: ['invoices', activeWorkspace?.id],
    queryFn: async () => {
      if (!activeWorkspace) return [];
      return await opsbrain.entities.Invoice.filter({ workspace_id: activeWorkspace.id });
    },
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!activeWorkspace) return [];
      return await opsbrain.entities.Transaction.filter({ workspace_id: activeWorkspace.id });
    },
    enabled: !!activeWorkspace
  });
  ...
  }, [invoices, clients, projects, transactions, activeWorkspace]);
      ...
      switch(insight.id) {
        case 'unpaid-invoices':
          for (const invoice of insight.data) {
            ...
            if (client?.email) {
              await opsbrain.integrations.Core.SendEmail({
                to: client.email,
        ...
        case 'inactive-clients':
          for (const client of insight.data.slice(0, 3)) {
            ...
            await opsbrain.entities.Task.create({
