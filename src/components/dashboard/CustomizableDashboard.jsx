...
export default function CustomizableDashboard() {
  ...
  const { data: widgets = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const existingWidgets = await base44.entities.DashboardWidget.filter({
        workspace_id: activeWorkspace.id,
        user_email: user.email,
        is_visible: true
      }, 'position');

      // אם אין ווידג'טים - צור אוטומטית את 3 הווידג'טים הבסיסיים
      if (existingWidgets.length === 0) {
      const defaultWidgets = [
        { widget_type: 'gmail', position: 0 },
        { widget_type: 'messages', position: 1 },
        { widget_type: 'calendar', position: 2 }
      ];
  ...
  const removeWidgetMutation = useMutation({
    mutationFn: async (widgetId) => {
      return await base44.entities.DashboardWidget.delete(widgetId);
    },
    onSuccess: () => {
