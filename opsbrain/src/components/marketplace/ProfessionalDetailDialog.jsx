...
export default function ProfessionalDetailDialog({ professional, open, onClose }) {
  ...
  const { data: reviews = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await opsbrain.entities.Review.filter({ professional_id: professional.id }, '-created_date');
    },
  ...
  const startChatMutation = useMutation({
    mutationFn: async () => {
      ...
      const existingChats = await opsbrain.entities.ProfessionalChat.filter({
      ...
      // צור צ'אט חדש
      const chat = await opsbrain.entities.ProfessionalChat.create({
        workspace_id: activeWorkspace.id,
        professional_id: professional.id,
        status: 'active',
      ...
      // שלח הודעה ראשונה
      if (message) {
        await opsbrain.entities.ChatMessage.create({
          chat_id: chat.id,
