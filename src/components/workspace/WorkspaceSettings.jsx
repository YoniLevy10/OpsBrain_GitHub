...
export default function WorkspaceSettings() {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ memberId, newRole }) => {
      return await base44.entities.WorkspaceMember.update(memberId, {
        role: newRole
      });
    },
    onSuccess: () => {
                ...
                <div className="space-y-4">
                  ...
                  <div>
                   ...
                   <Select value={inviteRole} onValueChange={setInviteRole}>
                     ...
                     <SelectContent>
                      <SelectItem value="viewer">{language === 'he' ? 'צופה - קריאה בלבד' : 'Viewer - Read only'}</SelectItem>
                      <SelectItem value="member">{language === 'he' ? 'חבר - צפייה ועריכה' : 'Member - View & edit'}</SelectItem>
                      <SelectItem value="admin">{language === 'he' ? 'מנהל - ניהול מלא' : 'Admin - Full management'}</SelectItem>
                     </SelectContent>
                   </Select>
                  </div>
