...
export default function InvoiceAutomation() {
  ...
  const { data: settings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const automations = await opsbrain.entities.Automation.filter({
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      if (settings?.id) {
        return await opsbrain.entities.Automation.update(settings.id, data);
      } else {
        return await opsbrain.entities.Automation.create({
          workspace_id: activeWorkspace.id,
