...
export default function Onboarding() {
  ...
  const handleUserMessage = async (userAnswer) => {
    ...
    if (currentQuestion >= questions.length - 1) {
      ...
      const completionMsg = {
        role: 'assistant',
        content: language === 'he'
          ? `מושלם! 🎉\n\nהגדרתי את המערכת בדיוק בשבילך כ-${typeConfig.he}.\nהמערכת עכשיו מותאמת אישית עם ${typeConfig.tabs.length} לשוניות ייחודיות לעסק שלך.\n\nמוכן להתחיל? 🚀`
          : `Perfect! 🎉\n\nI've configured the system just for you as a ${typeConfig.en}.\nThe system is now personalized with ${typeConfig.tabs.length} unique tabs for your business.\n\nReady to start? 🚀`
      };
      ...
      await createBusiness(updatedData);
    } else {
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      // עדכן את ה-workspace
      const workspaces = await opsbrain.entities.Workspace.filter({ id: workspaceId });
      if (workspaces.length === 0) {
        throw new Error('Workspace not found');
      }
      
      return await opsbrain.entities.Workspace.update(workspaceId, data);
    },
    onSuccess: () => {
