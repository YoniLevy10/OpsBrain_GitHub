...
export default function PermissionsManager() {
  ...
  const { data: permissionRecords = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await base44.entities.Permission.filter({ workspace_id: activeWorkspace.id });
    },
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (existing) {
        return base44.entities.Permission.update(existing.id, data);
      }
      return base44.entities.Permission.create(data);
    },
    onSuccess: () => {
