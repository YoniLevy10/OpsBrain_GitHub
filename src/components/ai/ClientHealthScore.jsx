...
export default function ClientHealthScore({ client }) {
  ...
  const { data: interactions = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!client) return [];
      return await base44.entities.Interaction.filter({ client_id: client.id }, '-date');
    },
    enabled: !!client
  });
  ...
  const { data: projects = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!client) return [];
      return await base44.entities.Project.filter({ client_id: client.id });
    },
    enabled: !!client
  });
  ...
  const { data: invoices = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!client) return [];
      return await base44.entities.Invoice.filter({ client_id: client.id });
    },
    enabled: !!client
  });
