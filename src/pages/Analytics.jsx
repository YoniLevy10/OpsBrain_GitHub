...
export default function Analytics() {
  ...
  const removeWidgetMutation = useMutation({
    mutationFn: (widgetId) => base44.entities.DashboardWidget.delete(widgetId),
    onSuccess: () => {
      queryClient.invalidateQueries(['dashboard-widgets']);
  ...
  const toggleWidgetMutation = useMutation({
    mutationFn: (widget) => base44.entities.DashboardWidget.update(widget.id, {
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600">
                    {language === 'he' ? 'סה"כ ווידג\'טים' : 'Total Widgets'}
                  </p>
