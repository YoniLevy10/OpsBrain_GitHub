...
export const WorkspaceProvider = ({ children }) => {
  ...
  const loadWorkspaces = async () => {
    try {
      ...
      // טען memberships לפי user_id (active)
      const activeMemberships = await base44.entities.WorkspaceMember.filter({
        user_id: user.id,
        status: 'active'
      });

      // טען גם הזמנות ממתינות לפי אימייל (invited) - למשתמשים שהוזמנו
      const pendingByEmail = await base44.entities.WorkspaceMember.filter({
        invited_email: user.email,
        status: 'invited'
      });

      // אשר הזמנות ממתינות - עדכן user_id ו-status ל-active
      ...
      for (const wsId of workspaceIds) {
        const ws = await base44.entities.Workspace.filter({ id: wsId });
        if (ws.length > 0) {
