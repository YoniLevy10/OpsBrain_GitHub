...
export default function Invoices() {
  ...
  const { data: invoices = [], isLoading: invoicesLoading } = useQuery({
    queryKey: ['invoices'],
    queryFn: () => opsbrain.entities.Invoice.list('-created_date'),
    staleTime: 1 * 60 * 1000,
  });
  ...
  const { data: business } = useBusinessQuery({
    queryKey: ['business'],
    queryFn: async () => {
      const businesses = await opsbrain.entities.Business.list();
      return businesses[0];
    }
  });

  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }) => opsbrain.entities.Invoice.update(id, { status }),
    onSuccess: () => {
